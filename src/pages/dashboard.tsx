...
import { useRoqPlatformUserProfiles, useReviewCount, useBookingCount, useCarCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const reviewStat = useReviewCount();

  const bookingStat = useBookingCount();

  const carStat = useCarCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Reviews',
      result: reviewStat?.data,
      isLoading: reviewStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Bookings',
      result: bookingStat?.data,
      isLoading: bookingStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Cars',
